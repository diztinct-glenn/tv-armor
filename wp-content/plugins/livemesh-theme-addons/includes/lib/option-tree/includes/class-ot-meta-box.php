...
if ( ! class_exists( 'OT_Meta_Box' ) ) {
	...
	class OT_Meta_Box {
		...
		public function build_meta_box( $post, $fields ) {
			...
			echo isset( $this->meta_box['desc'] ) && ! empty( $this->meta_box['desc'] ) ? '<div class="description" style="padding-top:10px;">' . wp_specialchars_decode( $this->meta_box['desc'] ) . '</div>' : ''; // phpcs:ignore
