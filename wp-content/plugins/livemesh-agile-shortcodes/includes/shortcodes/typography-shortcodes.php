...
function lags_sourcecode_shortcode($atts, $content = null, $shortcode_name = "") {
    return '<pre class="code">' . $content . '</pre>';
}

add_shortcode('code', 'lags_sourcecode_shortcode');
...
function lags_pullquote_shortcode($atts, $content = null, $shortcode_name = "") {
...
add_shortcode('pullquote', 'lags_pullquote_shortcode');
...
function lags_blockquote_shortcode($atts, $content = null, $shortcode_name = "") {
...
add_shortcode('blockquote', 'lags_blockquote_shortcode');
...
function lags_highlight_shortcode($atts, $content = null, $shortcode_name = "") {
...
add_shortcode('highlight1', 'lags_highlight_shortcode');
add_shortcode('highlight2', 'lags_highlight_shortcode');
...
function lags_list_shortcode($atts, $content = null) {
...
add_shortcode('list', 'lags_list_shortcode');
...
function lags_heading2_shortcode($atts) {
...
add_shortcode('heading2', 'lags_heading2_shortcode');
...
*/
function lags_segment_shortcode($atts, $content = null) {
    extract(shortcode_atts(array(
        'id' => '',
        'class' => '',
        'style' => '',
    ...
    if (!empty($style) || !empty ($background_image) || !empty($background_color) || !empty($background_pattern)) {
        ...
        $output .= '<div class="segment-content">' . do_shortcode(lags_remove_wpautop($content)) . '</div>';
        $output .= '</div><!-- .segment-->';
    }
    else {
        $output = '<div ' . $id . ' class="segment clearfix ' . $class . '"><div class="segment-content">' . do_shortcode(lags_remove_wpautop($content)) . '</div></div><!-- .segment-->';
    }

    return $output;
}

add_shortcode('segment', 'lags_segment_shortcode');
...
function lags_wrap_shortcode($atts, $content = null, $shortcode_name = "") {
...
add_shortcode('wrap', 'lags_wrap_shortcode');
add_shortcode('parent_wrap', 'lags_wrap_shortcode');
add_shortcode('ancestor_wrap', 'lags_wrap_shortcode');
...
[icon class="icon-shopping-cart" style="font-size:32px;"]

[icon class="icon-inbox" style="font-size:48px;"]
...
/* Shortcode for wrapping markup as visible in the visual editor. */
function lags_icon_shortcode($atts, $content = null, $shortcode_name = "") {
...
add_shortcode('icon', 'lags_icon_shortcode');
...
function lags_dummy_shortcode($atts, $content = null, $shortcode_name = "") {
...
add_shortcode('pricing_table', 'lags_dummy_shortcode');
...
*/
/* Shortcode for wrapping markup as visible for the dumb visual editor. */
function lags_action_call_shortcode($atts, $content = null, $shortcode_name = "") {
    ...
    $output .= '<h3>' . wp_specialchars_decode($text) . '</h3>';
...
add_shortcode('action_call', 'lags_action_call_shortcode');