...
if (!function_exists('mo_get_theme_skin')) {
    function mo_get_theme_skin() {
        ...
        if (empty($theme_skin)) {
            $theme_skin = mo_get_theme_option('mo_skin_color', '#fe5000');
        }
...
if (!function_exists('mo_generate_skin_styles')) {
    function mo_generate_skin_styles($skin_color) {

        if (empty($skin_color) || $skin_color === '#fe5000')
            return ''; // default theme color and hence nothing to change
...
#primary-menu > ul.menu > li > ul.sub-menu, #primary-menu > ul.menu > li:hover > ul.sub-menu { border-color: {$skin_color}; }
...
.team-member:hover h3 a, .post-snippets .hentry .entry-title a:hover { color: {$skin_color}; }
