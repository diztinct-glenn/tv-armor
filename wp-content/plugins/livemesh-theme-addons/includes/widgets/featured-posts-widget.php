...
class MO_Featured_Posts_Widget extends WP_Widget {
    ...
    public function __construct() {
        ...
        /* Widget settings. */
        $widget_ops = array('classname' => 'featured-posts-widget',
                            'description' => esc_html__('A widget that displays the featured posts.', 'livemesh-theme-addons'));

        /* Widget control settings. */
        $control_ops = array('width' => 300, 'height' => 350, 'id_base' => 'mo-featured-posts-widget');

        /* Create the widget. */
        parent::__construct('mo-featured-posts-widget', esc_html__('Featured Posts Widget', 'livemesh-theme-addons'), $widget_ops, $control_ops);
    }
    ...
    function widget($args, $instance) {
        ...
        $query_args = array('cat' => $featured_category, 'posts_per_page' => intval($post_count), 'ignore_sticky_posts' => 1);
    ...
    function form($instance) {
        ...
        <p>
            <label
                for="<?php echo esc_attr($this->get_field_id('featured_category')); ?>"><?php esc_html_e('Featured Category', 'livemesh-theme-addons'); ?>

                <select class="widefat" name="<?php echo esc_attr($this->get_field_name('featured_category')); ?>"
                        id="<?php echo esc_attr($this->get_field_id('featured_category')); ?>">
                    <?php
    ...
    function get_widget_defaults() {
        /* Set up some default widget settings. */
        $defaults = array('title' => esc_html__('Featured Posts', 'livemesh-theme-addons'),
                          'post_count' => '5',
                          'excerpt_count' => '100',
                          'featured_category' => '',
